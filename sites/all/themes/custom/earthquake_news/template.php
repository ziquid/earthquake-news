...
/**/
function earthquake_news_preprocess_html(&$variables) {
...
function earthquake_news_preprocess_views_view_unformatted(&$vars) {

  $view = $vars['view'];
  $rows = $vars['rows'];

  if ($view->name == 'top_5_articles' && $view->current_display == 'page_1')
	...
	if ($view->name == 'top_5_articles' && $view->current_display == 'block_1')
    foreach ($rows as $id => $row)
